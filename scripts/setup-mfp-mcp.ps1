...
Ok ("uv " + (uv --version))
if (-not (Get-Command claude -ErrorAction SilentlyContinue)) { Fail "Claude Code CLI ('claude') nicht gefunden." }
Ok ("Claude Code " + (claude --version))

# ---------------------------------------------------------------- 2. mfp-mcp installieren
Step 2 "mfp-mcp installieren und starten"
$help = uvx --from $FromSpec mfp-mcp --help 2>&1
if ($LASTEXITCODE -ne 0 -or ($help -notmatch "auth")) { Fail "uvx --from $FromSpec mfp-mcp --help schlug fehl:`n$help" }
Ok "uvx mfp-mcp startet (Version 0.3.0)"
if ($AutoRefresh) {
    uvx --from $FromSpec playwright install chromium
...
$cookiePath = uv run --quiet --with $Package python -c "from myfitnesspal_mcp import config; print(config.cookies_path())"
if (-not $SkipAuth) {
    Write-Host @"
...
"@
    ...
    uvx --from $FromSpec mfp-mcp auth
...
if ($AutoRefresh) {
    claude mcp add --scope user myfitnesspal -e "MFP_USERNAME=$Username" -- uvx --from "mfp-mcp[autorefresh]" mfp-mcp
} else {
    claude mcp add --scope user myfitnesspal -e "MFP_USERNAME=$Username" -- uvx mfp-mcp
}
...
uv run --with $Package python (Join-Path $RepoRoot "scripts\mfp_smoke_test.py")
...
Write-Host "  uv run --with $Package python scripts\mfp_smoke_test.py --write"
Write-Host "  uv run --with $Package python scripts\mfp_smoke_test.py --custom-food"
